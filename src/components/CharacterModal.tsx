  ...
  const filteredCharacters = React.useMemo(() => {
      ...
      if (searchQuery.trim()) {
          ...
          chars = chars.filter(c => {
              const nameMatch = normalize(c.name).includes(query);
              const titleMatch = c.title && normalize(c.title).includes(query);
  ...
  return (
      <Modal isOpen={isOpen} onClose={onClose} title="Personaje" accentClass={themeConfig.accentColor}>
         <form onSubmit={onSubmit} className="flex flex-col gap-4">
            ...
            {/* TOGGLE: CREATE OR LINK */}
            {['add-child', 'add-parent', 'add-partner'].includes(mode) && (
                 <div className="flex bg-zinc-950 p-1 rounded-lg mb-2 border border-zinc-800">
                    <button type="button" onClick={(e) => { e.preventDefault(); setIsLinkingExisting(false); }} className={`flex-1 text-xs py-1.5 rounded-md transition-all ${!isLinkingExisting ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>Crear Nuevo</button>
                    <button type="button" onClick={(e) => { e.preventDefault(); setIsLinkingExisting(true); }} className={`flex-1 text-xs py-1.5 rounded-md transition-all ${isLinkingExisting ? 'bg-zinc-800 text-white shadow-sm' : 'text-zinc-500 hover:text-zinc-300'}`}>Vincular Existente</button>
                 </div>
            )}

            {/* FORM FIELDS - ONLY SHOW IF NOT LINKING */}
            {!isLinkingExisting && (
