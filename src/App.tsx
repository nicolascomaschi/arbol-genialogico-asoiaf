...
export default function App() {
  ...
  const [formData, setFormData] = useState<{
    ...
    name: '', title: '', house: 'targaryen', isKing: false, isHeadOfHouse: false, isBastard: false, isNonCanon: false, isDragonRider: false, isDisputed: false, disputedParentageLore: '', dragonName: '', isGap: false, imageUrl: '', wikiLink: '',
    ...
    if (modalMode === 'edit') {
        ...
        setDatasets(prev => {
            ...
            const updatedHouse = {
                ...
                characters: currentHouse.characters.map(c => c.id === selectedCharId ? { 
                    ...
                    isKing: formData.isKing, isHeadOfHouse: formData.isHeadOfHouse, isBastard: formData.isBastard, isNonCanon: formData.isNonCanon, isDragonRider: formData.isDragonRider, isDisputed: formData.isDisputed, disputedParentageLore: formData.disputedParentageLore, dragonName: formData.dragonName, isGap: formData.isGap,
                    imageUrl: formData.imageUrl, wikiLink: formData.wikiLink, artistName: formData.artistName, artistLink: formData.artistLink, birthYear: formData.birthYear, deathYear: formData.deathYear, lore: formData.lore, status: formData.status as CharacterStatus
                } : c)
            };
        ...
        if (isLinkingExisting && linkCharId) {
             ...
             const newChar: Character = {
                    ...
                    generation: newGen, x: newX, house: 'targaryen', isKing: formData.isKing, isHeadOfHouse: formData.isHeadOfHouse, isBastard: formData.isBastard, isNonCanon: formData.isNonCanon, isDragonRider: formData.isDragonRider, isDisputed: formData.isDisputed, disputedParentageLore: formData.disputedParentageLore, dragonName: formData.dragonName, isGap: formData.isGap,
                    imageUrl: formData.imageUrl, wikiLink: formData.wikiLink, artistName: formData.artistName, artistLink: formData.artistLink, birthYear: formData.birthYear, deathYear: formData.deathYear, lore: formData.lore, status: formData.status as CharacterStatus
             };
  ...
  const openModalWrapper = useCallback((mode: typeof modalMode, charId: string) => {
    ...
    if (mode === 'add-root') {
         setFormData({ name: '', title: '', house: activeTab, isKing: false, isHeadOfHouse: false, isBastard: false, isNonCanon: false, isDragonRider: false, isDisputed: false, disputedParentageLore: '', dragonName: '', isGap: false, imageUrl: '', wikiLink: '', birthYear: '', deathYear: '', lore: '', status: 'alive', newHouseName: '', newHouseColor: 'black', newHouseCustomColor: '', artistName: '', artistLink: '' });
         return;
    }
    ...
    if (mode === 'edit' && char) {
        setFormData({ 
            ...
            isKing: !!char.isKing, isHeadOfHouse: !!char.isHeadOfHouse, isBastard: !!char.isBastard, isNonCanon: !!char.isNonCanon, isDragonRider: !!char.isDragonRider, isDisputed: !!char.isDisputed, disputedParentageLore: char.disputedParentageLore || '', dragonName: char.dragonName || '', isGap: !!char.isGap,
            imageUrl: char.imageUrl || '', wikiLink: char.wikiLink || '', artistName: char.artistName || '', artistLink: char.artistLink || '', birthYear: char.birthYear || '', deathYear: char.deathYear || '', lore: char.lore || '', status: char.status || 'alive',
            newHouseName: '', newHouseColor: 'black', newHouseCustomColor: ''
        });
    } else {
        setFormData({ name: '', title: '', house: activeTab, isKing: false, isHeadOfHouse: false, isBastard: false, isNonCanon: false, isDragonRider: false, isDisputed: false, disputedParentageLore: '', dragonName: '', isGap: false, imageUrl: '', wikiLink: '', birthYear: '', deathYear: '', lore: '', status: 'alive', newHouseName: '', newHouseColor: 'black', newHouseCustomColor: '', artistName: '', artistLink: '' });
        ...
        <div style={{ transform: `translate(${position.x}px, ${position.y}px) scale(${scale})`, transformOrigin: '0 0', width: `${CANVAS_SIZE}px`, height: `${CANVAS_SIZE}px` }} className="relative transition-transform duration-75 ease-linear">
           ...
           {visibleCharacters.length === 0 && characters.length === 0 && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="pointer-events-auto text-center"><h2 className="text-2xl font-cinzel font-bold text-zinc-500 mb-4">Vacío</h2><button onClick={() => setModalMode('add-root')} className="px-6 py-3 rounded-lg bg-zinc-800 text-zinc-600 font-cinzel border border-zinc-600">Crear Primer Personaje</button></div>
                </div>
           )}
